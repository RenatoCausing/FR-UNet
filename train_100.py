...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Train FR-UNet on the full patch set (base split plus holdout tiles) and evaluate on the test split.')
    parser.add_argument('-dp', '--dataset-path', required=True,
                        help='Path to the preprocessed ALL dataset (folder containing training_pro/holdout_pro/test_pro).')
    ...
    parser.add_argument('-o', '--output-root', default='runs/train_100',
                        help='Directory where pipeline artifacts will be written (default: runs/train_100).')
